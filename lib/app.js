...
function configure(app, port) {
  ...
  app.use(express.session({ store: new RedisStore({ host: config.get('CACHE_HOST') }), key: 'sessionId', cookie: { domain: config.get('COOKIE_DOMAIN'), path: '/', httpOnly: true, maxAge: config.get('SESSION_EXPIRATION_MS') }})); // TODO, test redis
